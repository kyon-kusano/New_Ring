...
public class AdminController extends selectBean {
	...
	public String test1(@PathVariable int page, @PathVariable String keyword, Model model, Model stringModel,
		...
		if (employees.size() == 0) {
			String errorString = "一致するユーザが見つかりません";
			stringModel.addAttribute("serchedKeyword", keyword);
			errorModel.addAttribute("error", errorString);
			model.addAttribute("employees", employees);
			model.addAttribute("pagination", pagination);
		} else {
			stringModel.addAttribute("serchedKeyword", keyword);
	...
	public String DataIntegrityViolationException(RedirectAttributes redirectAttributes) {
		redirectAttributes.addAttribute("error", "テスト");
		System.out.println("DataIntegrityViolationException occured");
	...
	@RequestMapping(value = "/error?error={error}", method = RequestMethod.GET)
	public String errorpage(@PathVariable("error") String error, Model model) throws Exception {
