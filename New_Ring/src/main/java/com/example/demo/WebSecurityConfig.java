...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
	    ...
	    //認証に関わるパラメータを指定
         http.formLogin().loginProcessingUrl("/login").loginPage("/login")  //ログインの処理をするURL  ログイン画面のURL
                         ...
                         .defaultSuccessUrl("/", true)                      //ログインが成功した時のURL(ここではindex.htmlなので"/"のみで指定可)  false : (認証されてなくて一度ログイン画面に飛ばされても)ログインしたら指定したURLに飛んでくれる
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		if(userService.findAllList().isEmpty()) {
		  userService.registerAdmin("admin", "secret", "admin@localhost", null, "男", "IT事業部", "090-2222-3333", "東京都", null, null, null);
		}
}
